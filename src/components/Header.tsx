            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className={`flex justify-between items-center transition-all duration-300 ${scrolled ? 'h-16' : 'h-20'
                    ...
                    <Link to="/" className="flex items-center space-x-3 group">
                        <img
                            src="/serviceorg-original.png"
                            alt="ES Structural Consultant"
                            className={`w-auto object-contain transition-all duration-300 ${scrolled ? 'h-10' : 'h-14'
                                } group-hover:scale-105`}
                        />
                        <div className="flex flex-col">
                            <span className={`text-primary-blue font-heading font-semibold leading-tight transition-all duration-300 ${scrolled ? 'text-base' : 'text-lg'
                                }`}>
                                ES Structural
                            </span>
                    ...
                    <nav className="hidden lg:flex items-center space-x-1">
                        {navLinks.map((link) => (
                            <Link
                                key={link.path}
                                to={link.path}
                                className={`relative px-4 py-2 rounded-md text-sm font-medium transition-all duration-300 group ${isActive(link.path)
                                        ? 'text-primary-blue'
                            ...
                            >
                                {link.name}
                                <span
                                    className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 bg-accent-green transition-all duration-300 ${isActive(link.path)
                                            ? 'w-3/4'
                    ...
                    <nav className="px-4 py-4 space-y-2">
                        {navLinks.map((link, index) => (
                            <Link
                                ...
                                className={`block px-4 py-3 rounded-lg text-sm font-medium transition-all hover:translate-x-2 ${isActive(link.path)
